...
const commandKeywordsMap = new Map<CommandKeyword, CommandType>(
  Object.entries(CommandDefinitions).map(([type, definition]) => [
    definition.keyword,
    type as CommandType,
  ])
);
...
function getCommandDefinitionFromKeyword(
  keyword: string
): [CommandType, CommandDefinition] | undefined {
  const commandType = commandKeywordsMap.get(keyword as CommandKeyword);
  return commandType
    ? [commandType, CommandDefinitions[commandType]]
    : undefined;
}
...
type CommandKeyword =
  (typeof CommandDefinitions)[keyof typeof CommandDefinitions]["keyword"];
